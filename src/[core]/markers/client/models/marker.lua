...
function Marker:activate()
    ...
    if self.on_enter then
        local succ, err = pcall(self.on_enter, self.interact_data)

        if not succ then
            Citizen.Trace("WARNING: Error executing on_enter for marker at " .. tostring(self.coords) .. ": " ..
                          tostring(err) .. "\n")
        end
    end
end
...
function Marker:render()
    ...
    if self.active and self.on_interact and IsControlJustPressed(0, 51) and
        ...
        local succ, err = pcall(self.on_interact, self.interact_data)
